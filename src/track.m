...
% Checking the input time vector is in ascending order (ie no negative time steps).
t   = xyzs( :, dd ) ;
dt  = diff( t ) ;

% Zero values are ok but negatives are not
if  ~isempty( find( dt < 0, 1 ) )
    disp( 'The time vector is not in ascending order. Format the input data in order of ascending aquisition time.' )
    return
end

if sum( dt ) == 0
    disp( 'There is nothing to track. All positions are at the same time.' )
    return
end
...
pos     = xyzs( eyes, 1 : dim ) ; % positionlist for first frame only.

n       = ngood ; % Number of particles for first frame.
zspan   = 50 ;
...
resx    = zeros( zspan, n ) - 1 ; % 2D Array of -1's of size, zspan, number of particles in first frame.

bigresx = zeros( frames, n ) - 1 ; % 2D Array of -1's of size, number of frames X number of particles in first frame.

mem     = zeros( n, 1 ) ; % 1D Array of 0's of size, number of particles in first frame

uniqid  = 1 : n ; % 1D array from 1 to the number of particles for first frame.
maxid   = n ; % Effectively, the number of particles for first frame.
olist   = [ 0, 0 ] ;
...
notnsqrd = ( sqrt( n * ngood ) > 200 ) & ( dim < 7 ) ; % sqrt( n * ngood ) == n
notnsqrd = notnsqrd( 1 ) ; % This is pointless since the above returns a single logical

if notnsqrd % if number of particles > 200 & number of input dimensions - 1 < 7
  ...
  % construct the vertices of a 3x3x3... d-dimensional hypercube
    ...
    blocksize = max( [ maxdisp, ( volume / 20 / ngood )^( 1 / dim ) ] ) ;
...
for i = 2 : z
    ...
    m       = res( i + 1 ) - res( i ) ; % equivalent to indicies( i ) - indicies( i - 1 ) which is to say, number of particles in current time window

    eyes    = 1 : m ; % Array from 1 to number of particles in current window
    
    eyes    = eyes + res( i ) ; % Array from particle 1 to last particle for current window 
    
    if m > 0
    
        xyi     = xyzs( eyes, 1 : dim ) ; % coordinates of all particles in current time window
            ...
            for nc = 1 : nclust
                ...
                if nnew > 5
                    ...
                    for ii = 1 : nnew
                        ...
                        if rnsteps > 2.e+5
                            disp( 'Excessive Combinitorics you FOOL LOOK WHAT YOU HAVE DONE TO ME!!!' )
                            return
                        end
                    end
                end
